...
```mermaid
graph LR
    subgraph "Drone Edge Zone"
        Drone["🚁 Drone Camera"] -->|"Wireless(RTSP)"| FastAPI["⚡ FastAPI AI Engine"]
        FastAPI -->|"Object & Pose Detection"| YOLO["🔥 YOLOv11 & Pose"]
    end

    subgraph "Backend Control Zone"
        FastAPI -->|"Event Alert(REST)"| SpringBoot["☕ Spring Boot API"]
        SpringBoot -->|"Data Logging"| MySQL[("🐬 MySQL")]
    end

    Client["📱 Dashboard"] <-->|"JWT Auth"| SpringBoot

    style Drone fill:#222,stroke:#00FFFF,stroke-width:2px,color:#fff
...
git clone [https://github.com/automaster5013/VisionFlow-Drone.git](https://github.com/automaster5013/VisionFlow_AI.git)
